...
**Leftover branches are reset; recorded states are not** (`branches_to_reset()`). `git worktree add <dest> <branch>` checks an existing branch out where it happens to sit, so a name reused across runs silently resurrected the previous one — `worktree delete` keeps branches by default, so this was the normal path. Verified before the change: after `delete runs` and a further commit in main, `add runs` gave a worktree whose superdataset HEAD was the old branch tip and whose `code/` still held the previous run's output, reported as a plain `2 created`.

The rule is decided across the hierarchy, because that is the only level at which the question has an answer. A branch present in *every* dataset is a state the hierarchy once recorded — the superdataset commit names the subdataset commits that belong with it — and going back to it is a deliberate act, so it is checked out untouched. A branch present in only *some* cannot describe any state: the datasets lacking it would branch off their current HEAD while the others returned to wherever the last run left them. Those are reset to their dataset's current HEAD (`git worktree add -B`), which is what makes a new worktree a fresh start.

Resetting moves a branch pointer, so `unmerged_branches()` refuses when the leftover holds commits its checkout lacks (`rev-list --count HEAD..<branch>`) — a finished run whose results were never fetched. `--force` overrides; `branch_beyond_head()` returning `None` (git could not answer) counts as blocking, since refusing on an unanswered question is the safe direction. The decision is computed before anything is created, so `--dry-run` names the branches it would move.

Each dataset's report says which of the three things happened — `(new branch)`, `(existing branch)`, `(leftover branch reset)` — because with an existing branch the two outcomes are materially different and neither should be silent.

The refusal is deliberately blunt: it also catches a branch created in *one* dataset on purpose and carrying real commits (a `variant` branch in the superdataset, say). To make a worktree of that, give the branch to every dataset — which is what makes it a state the hierarchy records — or `-f` to discard it. `tests/test_mtimes.py` documents the case.

Two limits worth knowing. "Present everywhere" does not prove the recorded state is self-consistent: if a subdataset's branch tip moved past the commit the superdataset's branch records, the new worktree is born with a dirty gitlink (verified: ` M code` immediately after creation). That is visible in `git status`, and re-saving the superdataset is the fix — it is not repaired here. And `--no-create-branch` asks for the branch as it stands, so it never resets.

**`--follow-parent` follows the recorded state** (issue #29, `recorded_hierarchy()` in `discovery.py`). A subdataset's branch tip and the commit its parent records for it are different things, and by default `add` uses the former — which is why a fresh worktree could be born with a dirty gitlink. With the flag, each subdataset is checked out at the commit its parent records; given a commit, the superdataset goes there too and the whole hierarchy mirrors that state.

One flag, with an optional commit, rather than two: "follow the parent" is the behaviour, and the commit merely says which state of the parent to follow.

The walk descends one dataset at a time, because `<commit>:sub-01/derivatives` cannot see inside `sub-01`'s tree — the gitlink for a nested dataset has to be read from *its own parent*, at the commit that parent is itself pinned to. Mutation-tested: resolving everything from the superdataset instead fails five tests.

With a commit the *set* of datasets comes from the commit too, not from the checkout: a subdataset added since is absent, one recorded then is included. A recorded dataset whose repository is missing locally is reported as not installed rather than silently dropped. And two things refuse before anything is created — an unresolvable commit, and a recorded commit the subdataset does not actually have (never fetched). Skipping the latter check lets git fail partway with "not a valid branch point", *after* the superdataset worktree exists.

**Replacement is the default** (issue #28; `-f` only discards). Worktrees are ephemeral, and a worktree that is merged or behind holds nothing but stale mtimes, so refusing to overwrite it bought nothing except a flag to type. What still refuses: a worktree holding commits the main checkout lacks (lifted by `-f`), and a destination git does not call a worktree (never lifted). `existing_worktrees()` lists only destinations git reports as worktrees, so a stray directory is never deleted. `unmerged_worktrees()` reuses `fetch.fast_forward_state()`: merged means `up-to-date` or `behind` — the state `worktree fetch` leaves behind — and a detached HEAD counts as unmerged. Replacement runs *before* the pre-flight, so the path and branch conflicts it would otherwise report are already gone; `_preflight_check(replaced_root=...)` additionally ignores conflicts pointing at the worktree being replaced, which is what makes `--dry-run` truthful. The branch is deleted too, so the recreate starts from the main checkout's HEAD instead of inheriting stale commits. Uncommitted changes are always discarded, matching `worktree delete --force`: the refusal is about commits, not dirt.
