...
> **Known gap.** All three guards identify the main working tree as `repo_path` — the dataset the command was resolved against, i.e. the tree you are standing in. Run from *inside* a linked worktree, `repo_path` is that worktree, so the real main checkout looks like just another linked worktree and both resolution modes delete it (verified: the `rmtree` fallback removes it after git refuses). The guards only hold when the command is run from the main checkout.
>
> The fix is to ask git per candidate path rather than infer from context: a path is a **linked** worktree exactly when `rev-parse --git-dir` differs from `rev-parse --git-common-dir`, and the main working tree of any dataset when they are equal. Two cheaper heuristics were considered and rejected. Testing whether `.git` is a directory works for DataLad-created hierarchies but tests the *layout*, not the property: DataLad keeps a subdataset's git dir in place, so `code/.git` is a directory (true for both `datalad create -d .` and `datalad clone` + `get -n`), whereas a subdataset added with plain `git submodule add` gets a gitlink *file* — `gitdir: ../.git/modules/code`, indistinguishable from a linked worktree's `.git` — as does any repo created with `--separate-git-dir`. Discovery walks `.gitmodules` and does not care which tool wrote the entry, and that misclassification is the destructive direction. (A linked worktree's `.git` is a plain file, never a symlink.) Second, "the first entry of `git worktree list --porcelain` is the main working tree" holds for the superdataset but reports a *submodule's* main working tree as `<super>/.git/modules/<name>`, so `<super>/code` never appears in the listing to be matched at all.
