...
- **Why merge, not rebase (fetch)**: not about conflicts -- a conflicting `merge` stops in a conflicted worktree exactly as a `rebase` does, and neither is ever reached because `merge_prediction()` predicts with `git merge-tree` in memory and refuses. Not about the dirty tree either: `rebase --autostash` handles that (same trick as jj, whose working copy *is* a commit), and measured, `stash`/`pop` re-times only the files that were already dirty. The real difference is that rebase replays under **new hashes**, and a superdataset records subdataset states *by hash* -- verified: after rebasing a subdataset, the commit the superdataset recorded is on no branch and is not an ancestor of the tip, so its recorded state stops being reproducible; after a merge it stays an ancestor. A fast-forward is neither operation: with no commits of our own git just moves the pointer.
...
- **`--follow-parent` takes the state from the parent's gitlink** (issue #29, `recorded_hierarchy()` in `discovery.py`): one flag with an optional commit, not two. Bare, subdatasets follow what the superdataset records now (this is what keeps a fresh worktree's gitlinks clean); with a commit, the superdataset goes there too and the commit also defines the *set* of datasets. The walk descends one dataset at a time because `<commit>:a/b` cannot see inside the submodule tree at `a` -- resolving everything from the superdataset fails five tests. Refuses before creating anything on an unresolvable commit or a recorded commit the subdataset lacks; without the latter check git dies mid-creation with "not a valid branch point" after the superds worktree already exists.
- **Branch logic is per-dataset for creation, hierarchy-wide for reuse**: if the branch is absent, create with `-b`. If it exists in *every* dataset, check it out as-is -- that is a state the hierarchy recorded. If it exists in only *some* (`branches_to_reset()`), those are leftovers from an earlier run and are reset to their dataset's HEAD with `-B`, because `worktree delete` keeps branches and reusing the name otherwise resurrected the previous run's code and outputs while the other datasets started fresh. `unmerged_branches()` refuses when a leftover holds commits its checkout lacks (`rev-list --count HEAD..<branch>`), `-f` overrides, and `branch_beyond_head() is None` counts as blocking. `--no-create-branch` never resets. The refusal is blunt on purpose: it also catches a branch created in one dataset on purpose and carrying commits (see `tests/test_mtimes.py`'s `variant` branch) -- give the branch to every dataset, or `-f`. Each report labels which happened: `(new branch)` / `(existing branch)` / `(leftover branch reset)`. Known limit: "present everywhere" does not prove the state is self-consistent -- a subdataset tip past the recorded gitlink yields a worktree born with ` M <sub>`.
...
- **The main working tree is never a delete target** (`is_main_worktree()`): `git worktree list` reports it alongside the linked ones, so a branch lookup landed on it, `git worktree remove` refused it, and the `rmtree` fallback then deleted the dataset outright — reported as `DELETED`. Annexed datasets survived by accident, because `rmtree` trips on mode-555 annex object directories; a text2git or no-annex dataset (what `code/` datasets are) was destroyed. Guarded in three places: the branch lookup skips it, path mode reports it as not-a-worktree, and `_git_worktree_remove` refuses it outright — the last being what makes the `rmtree` fallback safe at all. `add -f` routes through `delete_nested_worktrees`, so the same guard covers replacement.
- **Mainness is asked of git per path, not inferred from context** (`_worktree_kind()` in `delete.py`): a linked worktree's `--git-dir` is `<common-dir>/worktrees/<name>`, a main working tree's `--git-dir` *equals* `--git-common-dir`. Comparing the candidate against `repo_path` instead only holds when that dataset is the main checkout — run from *inside* a worktree it made the real main checkout look linked, and both resolution modes deleted it. `"unknown"` (git finds no repo there) is not refused, so stale registrations stay cleanable. Do not "simplify" this to a `.git`-is-a-directory test: it fails both ways (a plain `git submodule add` subdataset has a gitlink *file*; git-annex makes a worktree's `.git` a *symlink* to `<main>/.git/worktrees/<name>`, which `is_dir()` follows) and mutation-testing catches it in four tests.

### Result Types

- `WorktreeResult` (enum): `CREATED`, `CREATED_NEW_BRANCH`, `CREATED_RESET_BRANCH`, `SKIPPED_NOT_INSTALLED`, `SKIPPED_NOT_GIT_REPO`, `SKIPPED_DRY_RUN`, `SKIPPED_NO_WORKTREE`, `SKIPPED_CONTAINER`, `SKIPPED_UP_TO_DATE`, `CONFIGURED`, `MTIMES_SYNCED`, `FETCHED`, `DELETED`, `DELETED_BRANCH`, `FAILED`
